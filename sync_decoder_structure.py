...
for r in [5, 10, 15]:
    ...
    comp_param = [torch.randn((5, r), device=device), torch.exp(torch.randn((5, r), device=device))]
    samplebox = SampleBox(d, r, t, kappa, mix_param, comp_param, device, seed).to(device)
    xt, ct = samplebox.generate_xnc(20000, c_tran, x_tran)
    muc, logvarc = prior(ct)
    mean, logvar = encoder(xt, ct)
    var_ratio = torch.exp(logvar - logvarc).mean(dim=0)
    print(var_ratio)
    count1 = (var_ratio < 0.5).sum()
    ad_dot5.append(count1.tolist()+t)
...
np.save(f"{save_path}/../ad_dot05.npy", np.array(ad_dot05))
np.save(f"{save_path}/../ad_dot5.npy", np.array(ad_dot5))
for r in ["5","1","05"]:
    np.load(f"synthetic/prior_in_decoder/False/ad_dot{r}.npy")
...
for r in [5, 10, 15]:
    ...
    mean, logvar = encoder(xt, ct)
    var_ratio = torch.exp(logvar).mean(dim=0)
    print(var_ratio)
    count1 = (var_ratio < 0.5).sum()
    ad_dot5.append(count1.tolist()+t)
    count1 = (var_ratio < 0.1).sum()
