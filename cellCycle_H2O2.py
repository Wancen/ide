...
# df = pd.read_csv('../data/cellCycle/cell_cycle_H2O2.csv', header=None, names=['col{}'.format(i) for i in range(1, 48)])
df = pd.read_csv('../data/cellCycle/20211210-1445_knn150_t10_gamma0.25_allconditions.csv')
df_H2O2 = df[["AreaShape_Area_cell","Int_Med_cycD1_nuc",
...
df['Condition'].unique()
df_H2O2 = df_H2O2[df["Condition"].str.contains("H2O2")]

df_standardized = df_H2O2.apply(zscore)
...
lr = 0.005
coefs = [-20, -10, 0]
# c = torch.ones(df_H2O2.shape[0], device= device).unsqueeze(1)
numeric_values = [int(s.split('-')[1]) for s in df["Condition"][df["Condition"].str.contains("H2O2")].values]
c = torch.tensor(torch.nn.functional.one_hot(torch.tensor([x - 1 for x in numeric_values]), t), device=device)
c = c.to(torch.float32)
x = torch.tensor(df_standardized.values, device= device)
x = x.to(torch.float32)

for coef in coefs:
    save_path = f"cellCycle/param_prior_relu/H2O2/{coef}_{kappa}_{t}"
...
np.save(f"{save_path}/../H2O2_cvae_dot1.npy", np.array(ad_dot1))
np.save(f"{save_path}/../H2O2_cvae_dot05.npy", np.array(ad_dot1))
